#define LED_COUNT 10
#define ALWAYS_ON_PIN A5
#define BTN_PIN A3

#define HOLD_STEP 4

const int pins[] = {12, 4, 5, 6, 7, 8, 9, 10, 11, 2};

bool isPressing = false;
float minPressTime = 100;

int currentLED = 0;

int currentStep = 0;

float totalTimeToFill = 10000;
unsigned long startTime = 0;

int numberOfStrikes = 100;
int currentNumberOfStrikes = 0;
...
int noRest[] = {
  NOTE_FS4, NOTE_A4, NOTE_CS5, NOTE_A4, NOTE_FS4, NOTE_D4, NOTE_D4, NOTE_D4, NOTE_CS4
};
...
void playSong() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    setAll(melody[thisNote] != REST && melody[thisNote] != 8 && melody[thisNote] != 4);
...
void setup() {
  ...
  pinMode(ALWAYS_ON_PIN, INPUT_PULLUP);
...
void pressBtn(){
  ...
  if(pressed != isPressing){
    if(pressed){
      ...
      if(millis() > (startTime + minPressTime)){
        increaseProgress();
      }
    }
...
void loop() {
  ...
  if(digitalRead(ALWAYS_ON_PIN) == HIGH){
    currentLED = 0;
    setAll(LOW);
    return;
  }

  if (currentStep < HOLD_STEP) {
    pressBtn();
  }

  if (currentStep == HOLD_STEP) {
    if (digitalRead(BTN_PIN) == LOW && digitalRead(ALWAYS_ON_PIN) == LOW) {
      totalTimeToFill = 10000;
      currentNumberOfStrikes = 0;
      startTime = millis();
      currentLED = 0;
      activateLED(currentLED); 
      while (currentNumberOfStrikes < numberOfStrikes) {
        if(digitalRead(BTN_PIN) != LOW && digitalRead(ALWAYS_ON_PIN) != HIGH){
          currentNumberOfStrikes += 1;
          continue;
        }
        
        currentNumberOfStrikes = 0;
        float time = millis() - startTime;
        int indexes = (time / totalTimeToFill) * 10;

        if (indexes > currentLED) {
          activateLED(currentLED);

          // we only play the note for 90% of the duration, leaving 10% as a pause
          tone(buzzer, noRest[currentLED + 1], 8);
          ...
          if(currentLED > 2){
            totalTimeToFill += 1000;
          }

          if(currentLED > 4){
            totalTimeToFill += 2000;
          }

           if(currentLED > 4){
            totalTimeToFill += 3000;
          }
